...
# Identifers - variables - operators - control statements - strings
...
while not student_id_valid:
    ...
    # check for negative numbers (-10) (-digit) (-10)
...
formated_id = "EDIFY-" + str(student_id)
print(formated_id)
...
while not student_name_valid:
    ...
    # Remove any spaces at beginning and ending and also we title cased
    student_name = student_name.strip().title()
    
    # validate given name
    name_check = student_name.replace(" ","") # remove space in between name
    
    # is name alphabet only and also at least 3 characters
...
# if "PROMO" is coupon-> string approach
if coupon.find("PROMO") != -1:
    discount = 5000
    
# if "PROMO" in coupon -> operator approach
# if "PROMO" in coupon:
#     discount = 5000
